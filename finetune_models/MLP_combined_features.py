...
def get_inputs(inp_dir, dataset, embed, embed_mode, mode, layer):
    ...
    other_features = pd.DataFrame(np.array(other_features_df))
    print('other_features shape: ', np.array(other_features).shape)
    ...
    for trait_idx in range(full_targets.shape[1]):
        ...
        for train_index, test_index in skf.split(inputs, targets):
            ...
            model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=lr),
                          ...
                          BinaryCrossentropy(from_logits=True), metrics=['mse','accuracy'])

            # dataSet = np.array(x_train)
            # print(dataSet)
            # n = np.shape(dataSet)
            # print(n)
            # dataSet1 = np.array(y_train)
            # m = np.shape(dataSet1)
            # print(m)
